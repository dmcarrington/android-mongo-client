...
fun SettingsScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            SectionHeader("Browse defaults")
            Row(
                modifier = Modifier.fillMaxWidth(),
                verticalAlignment = Alignment.CenterVertically,
            ) {
                Column(modifier = Modifier.weight(1f)) {
                    Text("Show system databases by default", style = MaterialTheme.typography.bodyLarge)
                    Text(
                        "Includes admin, local, and config when listing databases.",
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
                }
                Switch(
                    checked = state.showSystemDbsDefault,
                    onCheckedChange = viewModel::setShowSystemDbsDefault,
                )
            }
            ...
            Text(
                "A MongoDB client for Android. Connection URIs and embedded credentials are encrypted at rest using Android Keystore (AES/GCM). See README for details.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
        }
